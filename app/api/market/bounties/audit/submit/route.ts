...
import { submitBountyAudit } from "@/lib/server/market";
...
export async function POST(request: Request) {
  const rl = applyRateLimit(request, { key: "market_audit_submit_v1", limit: 40, windowMs: 5 * 60 * 1000 });
  ...
  if (decision !== "confirm" && decision !== "reject") {
    return NextResponse.json({ error: "Invalid decision (must be confirm|reject)." }, { status: 400, headers: { "Cache-Control": "no-store" } });
  }

  try {
    const bounty = await submitBountyAudit({
      id,
      handle,
      decision,
    ...
    const message = err instanceof Error ? err.message : "Audit submit failed.";
