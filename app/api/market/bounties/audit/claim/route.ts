...
import { claimBountyAudit } from "@/lib/server/market";
...
export async function POST(request: Request) {
  const rl = applyRateLimit(request, { key: "market_audit_claim_v1", limit: 40, windowMs: 5 * 60 * 1000 });
  ...
  try {
    const bounty = await claimBountyAudit({ id, handle });
    return NextResponse.json({ bounty }, { headers: { "Cache-Control": "no-store" } });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Audit claim failed.";
